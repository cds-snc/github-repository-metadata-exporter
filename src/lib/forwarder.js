...
const buildSignature = (customerId, sharedKey, date, contentLength, method, contentType, resource) => {
    ...
    let encodedHash = crypto.createHmac('sha256', decodedKey).update(bytes).digest('base64')
...
const postData = async (customerId, sharedKey, body, logType) => {
    ...
    let signature = buildSignature(customerId, sharedKey, rfc1123date, contentLength, method, contentType, resource)
